...
class AddMonsterForm extends Component {
  ...
  validateMonster = (monster) => {
    ...
    if (hp < 1 || hp > 1000000) {
      ...
      MonsterApiService.postMonster(monster)
        .then((res) => (res.id ? history.goBack() : ''))
  ...
  renderInputs = () => {
    const FIELDS = [
      ...
      { name: 'morph', label: 'Morph', defaultValue: 'N/A' },
  ...
  render() {
    ...
    return (
      <div className="add-monster-container">
        ...
        <form
          name="add-monster-form"
          id="add-monster-form"
          onSubmit={this.handleSubmitForm}
        >
          <div className="form-description">
            All fields are required. If monster has no
            weakness/strength/drop/steal, simply put N/A as the value for that
            field.
          </div>
          <button type="button" onClick={this.handleCancelForm}>
            Not Interested
          </button>
          <div className="all-the-inputs">{this.renderInputs()}</div>
